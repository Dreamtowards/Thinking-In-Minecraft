...
上一章（[《存储与序列化》](/impl/storage)）把柱子写进文件。柜子装的是已经生成的法律。本章把法律从噪声里叫醒。叫醒有阶段。阶段有邻居。邻居一乱，确定性就露出边界。
...
**噪声**给连续的起伏：哪实心、哪空、哪是水。1.18 之后，主世界用三维密度：某格 `final_density > 0` 放默认石头，否则交给空气或含水层。[^density] 它不管村庄的事，只负责给走路的人一块站得住的地板。

**群系**在地形还没堆起来之前，就可以先被采样。Java 的步骤里，`biomes` 早于 `noise`：先用气候参数（大陆性、侵蚀、温度、湿度、奇怪度）定下这根柱子是哪种群系，再决定表面铺草还是铺沙。[^steps] 能说「往南是沙漠」，探索才有口语。
...
Java 把这三层收成可数据驱动的 JSON：噪声设置、密度函数、表面规则、配置特征、结构集。1.16.2 起数据包能改它们。[^datapack] Bedrock 把近似的职责写进行为包与实验性开关，口径不是同一份 schema（见[《命令、数据包与地图》](/design/commands-datapacks)）。作者席开在数据层。数据层改得了形状，改不了「失败就当没发生」这条脾气。
...
Java 把一块从空变成可玩，排成一条公开写在区块 NBT `Status` 里的梯子。远的柱子可以冻在半截，叫原型区块（proto-chunk）；玩家走近，再往上爬。爬完全部步骤才转正成完整区块，攒下的方块更新一次性倒出来。[^steps]
...
示意不是调度源码。它要说明：同样的种子，若两台机器让不同的区块先 `FEATURES`，两棵树仍可能抢边。抢边就是本章末尾要钉的确定性边界。这里先承认生命周期不是一次性函数，是可暂停的状态机。暂停，是为了远的柱子别把 CPU 吃光。玩家走到哪，哪一圈才配从噪声爬到满。
...
结构想要一块平地。噪声不保证平地。旧村庄会悬空、埋进山、桥接到悬崖。设计上这不叫失败，叫「假装地形配合过」。零件放下去，底下是空气，它就悬在空气上，不会替你补一块地基。玩家把悬空当成风景，把埋进山当成地牢。合同允许错过，也允许尴尬。
...
结构集用间距、分隔、频率在网格上掷点；点落在错误群系就整座不放。[^struct] 要塞走同心环，是这条规则的例外。例外证明：大多数结构接受「这颗种子就是没有」。没有，是可探索。保底，是可通关。生成器默认选前者。
...
示意：每个特征要自己的盐，免得整块装饰共用一个随机流。盐配得不好，同一批结构会在地图上排成可预测的对角线。重复是伪随机的齿，不是种子坏了。开发者若把 `Random` 直接绑在世界种子上、不按块、不按步加盐，会得到条纹矿和成排的树。加盐，仍解决不了邻居抢写。
